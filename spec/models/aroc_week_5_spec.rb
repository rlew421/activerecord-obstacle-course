...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # Which ones are you able to switch around without relying on Ruby's Enumerable methods?
      ...
      # ordered_items_names = Item.joins(:orders).distinct.order(:created_at).pluck(:name)
      # ^ The above solution also works

      # ordered_items_names = Item.joins(:orders).order(:created_at).pluck(:name).distinct
      # ^ Doesn't work because you can't call distinct on an array. You can't call distinct after pluck because when you pluck the names, what's returned
      # is no longer an ActiveRecord relation, it's an array.
    # ---------------------------------------------------------------
